...
class dqn(algorithm.Algorithm):
    def __init__(self, session, name='dqn', version='2013', gameparam={}, externalHyperparam={}):
    ...
    def setDefaultHyperparam(self):
        ...
        hyperparams['hidden_layer'].append({'layer_name':'W1', 'size':{'dtype':'integer', 'variable':False, 'value':30, 'minvalue':2, 'maxvalue':1000} })
    ...
    def buildNetwork(self):
        with tf.variable_scope(self.net_name):
            ...
            '''if 'hidden_size_1' in self.hyperparams:

                W1 = tf.get_variable("W1", shape=[self.gameparams['input_size'], self.hyperparams['hidden_size_1']['value']],
                                     initializer=tf.contrib.layers.xavier_initializer())
                L1 = tf.nn.tanh(tf.matmul(self._X, W1))

            W2 = tf.get_variable("W2", shape=[self.hyperparams['hidden_size_1']['value'], self.gameparams['output_size']],
                                 initializer=tf.contrib.layers.xavier_initializer())
        ...
        #self._Y = tf.placeholder(dtype=tf.float32, shape=[None, self.gameparams['output_size']], name="output_Y")
        #self._Y = tf.placeholder(dtype=tf.float32, shape=[None, self.gameparams['output_size']])
    ...
    def predict(self, state):
        ...
        return self.session.run(self._train, feed_dict={self._X: x})
    ...
    def replay_memory_batch(self, replay_batch):
        ...
        for state, action, reward, next_state, done in replay_batch:
            ...
            if done:
                ...
                Q[0, action] = reward + self.hyperparams['discount_ratio'] * np.max(self.predict(next_state))
