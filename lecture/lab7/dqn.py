...
class dqn(algorithm.Algorithm):
    def __init__(self, session, name='dqn', version='2013', externalHyperparam={}):
        self.session = sess
        self.version = version
        self.net_name = name
        self.hyperparams = self.__setDefaultHyperparam()
        self.hyperparams.update(externalHyperparam)

        self.network = self.__buildNetwork()

    def __setDefaultHyperparam(self):
        self.hyperparams['input_size'] = 30
        self.hyperparams['output_size'] = 4
        self.hyperparams['hidden_size_1'] = 30


    def __buildNetwork(self):
        with tf.variable_scope(self.net_name):
            self._X = tf.placeholder(dtype=tf.float32, shape=[None, self.hyperparams['input_size']], name="input_X")

            W1 = tf.get_variable("W1", shape=[self.hyperparams['input_size'], self.hyperparams['hidden_size_1']],
                                 initializer=tf.contrib.layers.xavier_initializer())
            L1 = tf.nn.tanh(tf.matmul(self._X, W1))

            W2 = tf.get_variable("W2", shape=[self.hyperparams['hidden_size_1'], self.hyperparams['output_size']],
                                 initializer=tf.contrib.layers.xavier_initializer())

            self._QPred = tf.matmul(L1, W2)

        self._Y = tf.placeholder(dtype=tf.float32, shape=[None, self.hyperparams['output_size']], name="output_Y")
